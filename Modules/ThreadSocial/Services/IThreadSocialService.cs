...
namespace YopoBackend.Modules.ThreadSocial.Services
{
    public interface IThreadSocialService
    {
        ...
        Task<(List<ThreadPostResponseDTO> Posts, int TotalRecords)> GetPostsAsync(int userId, string userRole, int? buildingId, int? authorId, string? search, int page, int pageSize);
        ...
        Task<ThreadPostResponseDTO> UpdatePostReactionAsync(int postId, int userId, string userRole, string? reaction);
